...
SELECT t.id, t.task_name, MIN(s.start_time) AS start_time, MAX(s.end_time) AS end_time
FROM tasks t
...
SELECT t.id, t.task_name, MIN(s.start_time) AS start_time, NULL::timestamptz AS end_time
FROM tasks t
JOIN tasks_sessions s ON s.task_id = t.id
GROUP BY t.id, t.task_name
HAVING COUNT(*) FILTER (WHERE s.end_time IS NULL) > 0
...
SELECT t.id, t.task_name, t.is_paused,
...
WHERE t.id = $1
GROUP BY t.id, t.task_name, t.is_paused;

-- name: CreateNewTask :one
INSERT INTO tasks (task_name) VALUES ($1) RETURNING id, task_name, is_paused;
...
UPDATE tasks_sessions SET end_time = $1
